...
def _create_redis_client(host: str, port: int, conn_timeout: int):
...
class SignalExporter:
    ...
    def __init__(
        ...
        Raises redis.ConnectionError if a redis server connection cannot be
        established.
        """
        ...
        self.redis = _create_redis_client(redis_host, redis_port, conn_timeout)
            ...
            if data and data["publisher_id"] == self.pub_id and data["event"] == event:
                if "ras" in data:
                    if data["responder_id"] not in to_check:
                        self.logger.warning(
                            f"Got a response from tool '{data['responder_id']}'"
                            " but it's not on the known subscribers list (or"
                            f" already responded for '{event}')."
                            f" RAS: {data['ras']}"
                        )
                    else:
                        ...
                        if data["ras"] != 1:
                            self.logger.warning(
                                f"Tool '{data['responder_id']}' returned bad"
                                f" response for event '{event}', ras: {data['ras']}"
                            )
